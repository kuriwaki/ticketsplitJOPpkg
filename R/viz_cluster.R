...
get_mus <- function(object) {
  ...
  # run and bind
  bind_rows(lapply(seq_len(n_levels), function(x) get_mu_ell(outcome = x, data, names = office_names)))
...
#'
#' @rdname get_mus
get_mu_ell <- function(outcome, data, names) {
  ...
  mu_df <- as_tibble(data$mu[, , outcome], .name_repair = \(x) names)
